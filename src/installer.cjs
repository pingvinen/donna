...
async function run(options = {}) {
    ...
    if (detected.length > 0) {
        for (const provider of detected) {
            ...
            output.success(
                `Copied donna skills (setup, add-task, done, set-role, begin-the-day, add-tool, remove-tool, relearn-tools, run-tools, help, contribute-idea, adjust-tool, focus, follow-up) to ${provider.stubTarget}`,
            );
        }
    } else {
